...
const heightResize = () => {
    ...
    document.querySelector('.menu').style.height = menuContentsHeight + 'px';
    document.querySelector('#contents').style.height = menuContentsHeight + 'px';
};
    ...
    try {
        ...
        response.data.forEach(res => {
            ...
            infoItem.querySelector('.info-item-summary').addEventListener('click', () => {
                if (openedItem === infoItem) {
                    contentClose(infoItem);
                    openedItem = null;
                } else {
                    if (openedItem) contentClose(openedItem);
            ...
            infoItem.querySelector('#delete').addEventListener('click', async () => {
                if (confirm(message.infoDeleteConfirm)) {
                    try {
                        await restHandler('DELETE', '/deleteInfo', { id: res.id });
                        alert((itemNumber - 1) + '번 ' + message.infoDeleteComplete);
                        location.reload();
                    } catch (error) {
            ...
            infoItem.querySelector('#modify').addEventListener('click', () => {
                writeInfo(openedItem);
            });
...
const writeInfo = async () => {
    ...
    const importPage = async (target, contents) => {
        ...
        if (openedItem) {
            insertDataInSummernote(openedItem);
...
const putInfo = async () => {
    let data = {
        id: parseInt(openedItem.id.substr(4)),
        title: document.querySelector('#title-info').value,
        content: document.querySelector('#summernote').value
    };
    alert(message.updateProgressing);
    try {
